...
navDatabaseInitializer.prototype.init = function () {
    ...
    return this.dbQuery('CREATE TABLE IF NOT EXISTS nav_user( _id varchar(36) NOT NULL, first_name text, last_name text, email_address varchar(30), mobile_no varchar(15), password text, email_verification VARCHAR(36), subscribed_plan VARCHAR(10), points integer,balance integer, deposit integer, address text, city varchar(50), state varchar(30), is_active smallint, user_type smallint, CONSTRAINT nav_user_id_pk PRIMARY KEY (_id));')
        ...
        .then(() => {
            return self.dbQuery('CREATE TABLE IF NOT EXISTS nav_rentals( _id VARCHAR(36), user_id varchar(36), toys_id varchar(36), lease_start_date bigint, lease_end_date bigint, shipping_address TEXT, CONSTRAINT nav_rental_id_pk PRIMARY KEY (_id),CONSTRAINT nav_rentals_toys_id_fk FOREIGN KEY (toys_id) REFERENCES  nav_toys (_id) MATCH FULL ON DELETE CASCADE ON UPDATE CASCADE NOT DEFERRABLE,CONSTRAINT nav_rentals_user_id_fk FOREIGN KEY (user_id) REFERENCES  nav_user (_id) MATCH FULL ON DELETE CASCADE ON UPDATE CASCADE NOT DEFERRABLE); ');
        })
        .then(() => {
