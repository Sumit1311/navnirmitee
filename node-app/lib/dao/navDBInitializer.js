...
navDatabaseInitializer.prototype.init = function () {
    var self = this;
    return this.dbQuery('CREATE TABLE IF NOT EXISTS nav_user( _id varchar(36) NOT NULL, first_name text, last_name text, email_address varchar(100), mobile_no varchar(15), password text, email_verification VARCHAR(36), subscribed_plan VARCHAR(10), points integer,balance integer DEFAULT 0, deposit integer DEFAULT 0, address text, city varchar(50), state varchar(30), membership_expiry bigint, enrollment_date bigint, pin_code VARCHAR(10), user_type smallint, CONSTRAINT nav_user_id_pk PRIMARY KEY (_id));')
        .then(function () {
        ...
        .then(() => {
            return self.dbQuery('CREATE TABLE IF NOT EXISTS nav_toys (_id varchar(36), name varchar(50), stock integer, price integer, short_description VARCHAR(100), long_description TEXT, points integer, age_group smallint, parent_toys_id varchar(36), rent_duration integer, brand integer, category smallint, CONSTRAINT nav_toys_id_pk PRIMARY KEY (_id), CONSTRAINT nav_toys_parent_toys_id_fk FOREIGN KEY (parent_toys_id) REFERENCES  nav_toys (_id) MATCH FULL ON DELETE CASCADE ON UPDATE CASCADE NOT DEFERRABLE)');
        })
        .then(() => {
